		...
		<form method="post" action="#">
			<fieldset>
				...
				try{
					...
					$lista = $con->prepare(
						"SELECT j.idjogo, j.titulojogo, j.resumojogo, 
                        j.valorjogo,
                        c.desccons, s.descsh FROM tb_jogo j, 
                        tb_console c, tb_softhouse s 
                        WHERE 
                        j.fk_softhouse = s.idsh AND
                        j.fk_console = c.idcons 
					...
					foreach($lista as $linhas){
						echo "<tr>
                                ...
                                <td> " . $linhas[5] . " </td>
                                <td> " . $linhas[4] . " </td>
								<td> <a href='altJogo.php?cod=" . $linhas[0] . "'> Editar </a> </td>
								<td> Exclui </td>
							  </tr>";
					}
				}
